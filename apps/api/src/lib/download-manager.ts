...
import { type Download as DownloadRecord, prisma } from '@rip/db'
...
class DownloadManager {
  ...
  async queueDownload(userId: string, payload: QueueDownloadRequest) {
    ...
    const id = await this.mutex.run(async () => {
      const count = await prisma.download.count({
        where: {
          status: {
            in: ['queued', 'downloading', 'processing'],
          },
        },
      })

      if (count >= env.maxIncompleteDownloads) {
        return null
      }

      const inserted = await prisma.download.create({
        data: {
          userId,
          url: payload.url,
          title: payload.title,
  ...
  private async worker(_index: number) {
    while (this.started) {
      ...
      try {
        ...
        // Errors are persisted by runDownload.
      }
    }
  }
  ...
  private async runDownload(row: DownloadRecord) {
...
function mapRowToDownloadItem(row: DownloadRecord): DownloadItem {
  return {
    id: row.id,
    userId: row.userId,
    url: row.url,
